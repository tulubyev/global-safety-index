...
const KEYS: (keyof Weights)[] = ['conflict', 'crime', 'disaster', 'food', 'seismic', 'pandemic'];
...
export function normalizeWeights(weights: Weights): Record<keyof Weights, number> {
  ...
  if (sum === 0) return { conflict: 0.25, crime: 0.2, disaster: 0.15, food: 0.15, seismic: 0.1, pandemic: 0.15 };
